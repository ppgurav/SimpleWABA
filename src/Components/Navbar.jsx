...
//     // Close contact dropdown when collapsing sidebar
//     if (isOpen) {
    ...
    return (
      <Link
        ...
        className={`
          ...
          ${isOpen ? "gap-3 px-4 py-3 mx-2" : "justify-center p-3 mx-2"}
          ${
            isActive
              ? "text-white bg-gradient-to-r from-blue-500 to-purple-600 shadow-lg shadow-blue-500/25 font-semibold border-l-4 border-yellow-400"
              : "text-gray-700 hover:text-blue-600 hover:bg-white hover:shadow-md hover:shadow-gray-200/50 bg-gray-50/50"
          } ${className}
        `}
        title={!isOpen ? children : ""}
      >
        <Icon size={20} className="flex-shrink-0 drop-shadow-sm" />
        <span className={`${!isOpen ? "hidden" : "block"} font-medium`}>{children}</span>
        {!isOpen && (
  ...
  const SectionHeader = ({ children }) => (
    <h2
      className={`px-4 py-2 text-xs font-bold text-gray-500 uppercase tracking-wider ${
        !isOpen ? "hidden" : "block"
      } bg-gradient-to-r from-gray-100 to-gray-50 rounded-lg mx-2 shadow-sm`}
    >
      {children}
    </h2>
  )
    ...
    <Menu size={20} />
      ...
      <div
        className={`
          ...
          border-r border-gray-200 shadow-2xl shadow-gray-500/10
          transform-gpu flex flex-col h-full
          transition-all duration-300 ease-in-out
      ...
      >
        {/* Logo section */}
        <div
          className={`flex items-center border-b border-gray-200 bg-white shadow-sm ${isOpen ? "justify-between p-6" : "justify-center p-4"}`}
        >
          <div className={`${!isOpen ? "hidden" : "block"}`}>
            <div className=" ">
          ...
          <button
            onClick={toggleSidebar}
            className={`p-2 rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 text-white hover:from-blue-600 hover:to-purple-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-110 active:scale-95 ${!isOpen ? "mt-2" : ""}`}
            aria-label="Toggle sidebar"
          >
            <Menu size={18} />
          </button>
        ...
        {/* Navigation */}
        <nav
          className={`flex-1 overflow-y-auto space-y-6 ${isOpen ? "p-4" : "p-2"} bg-gradient-to-b from-transparent to-gray-50/50`}
        >
          {/* Main Navigation */}
          <div className={`space-y-2 ${!isOpen ? "space-y-1" : ""}`}>
            {navigationItems.map((item) => (
          ...
          <div className={`space-y-2 ${!isOpen ? "space-y-1" : ""}`}>
            ...
            <div>
              {isOpen ? (
                <button
                  onClick={toggleContactDropdown}
                  className={`flex items-center gap-3 w-full px-4 py-3 mx-2 rounded-xl transition-all duration-300 transform hover:scale-105 active:scale-95 ${
                    isContactSectionActive
                      ? "text-white bg-gradient-to-r from-blue-500 to-purple-600 shadow-lg shadow-blue-500/25 border-l-4 border-yellow-400 font-semibold"
                      : "text-gray-700 hover:text-blue-600 hover:bg-white hover:shadow-md bg-gray-50/50"
                  }`}
                >
                  <User size={20} className="flex-shrink-0 drop-shadow-sm" />
                  <span className="flex-grow text-left font-medium">Contact</span>
                ...
                <Link
                  ...
                  onClick={() => handleLinkClick("/contact")}
                  className={`
                    flex items-center justify-center p-3 mx-2 rounded-xl transition-all duration-300 group relative transform hover:scale-105 active:scale-95
                    ${
                      isContactSectionActive
                  ...
                  {contactItems.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        to={item.to}
                        onClick={() => handleLinkClick(item.to)}
                        className={`flex items-center gap-3 px-4 py-2 mx-2 rounded-lg transition-all duration-300 transform hover:scale-105 active:scale-95 ${
                          isActive
                            ? "text-blue-600 bg-blue-50 border-l-2 border-blue-400 font-medium shadow-sm"
      ...
      <div className="flex flex-col h-screen">
        ...
        <nav
          className={`
            fixed top-0 right-0 h-20 bg-white/80 backdrop-blur-md border-b border-gray-200 shadow-lg
            flex items-center px-6 z-10 transition-all duration-300 ease-in-out
            ${isMobile ? "left-0 w-full" : isOpen ? "left-72 w-[calc(100%-18rem)]" : "left-20 w-[calc(100%-5rem)]"}
          `}
        >
          <div className="ml-auto flex items-center space-x-4">
            {/* Settings link */}
            <Link
              ...
              className={`p-3 rounded-xl transition-all duration-300 transform hover:scale-110 active:scale-95 ${
                activeLink === "/setup"
                  ? "text-white bg-gradient-to-r from-blue-500 to-purple-600 shadow-lg shadow-blue-500/25"
                  : "text-gray-600 hover:text-blue-600 hover:bg-white hover:shadow-md bg-gray-50"
              }`}
              title="Settings"
            >
              <Settings size={20} />
            ...
            <div className="relative dropdown-container ">
              <button
                ...
                className="bg-gradient-to-r text-white from-blue-500 to-purple-600  shadow-blue-500/25 p-3 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 active:scale-95 ring-2 ring-transparent rounded-full"
                title="Quick Access"
              >
                <Plus size={20} />
              </button>
              ...
              {isPlusMenuOpen && (
                <div className="absolute left-1/2 top-full mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden z-50 backdrop-blur-md transform -translate-x-1/2">
                    ...
                    {quickAccessItems.map((item, index) => {
                      ...
                      return (
                        <Link
                          key={index}
                          to={item.to}
                          className={`block px-4 py-3 text-sm transition-all duration-300 transform hover:scale-105 ${
                            isActive
                              ? "text-blue-600 bg-blue-50 font-medium border-l-4 border-blue-400"
                              : "text-gray-700 hover:bg-blue-50 hover:text-blue-600"
                          }`}
                          onClick={() => {
                            handleLinkClick(item.to)
            ...
            <div className="relative dropdown-container">
              <button
                // onClick={() => setIsDarkModeMenuOpen(!isDarkModeMenuOpen)}
                className="p-3 rounded-xl text-gray-600 hover:text-gray-900 hover:bg-white hover:shadow-md bg-gray-50 transition-all duration-300 transform hover:scale-110 active:scale-95"
                title="Theme"
              >
                {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
              </button>
              ...
              {isDarkModeMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden z-50 backdrop-blur-md">
                  <div className="py-2">
                    <button
                      onClick={() => toggleDarkMode("light")}
                      className="flex items-center gap-3 w-full px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-all duration-300 transform hover:scale-105"
                    >
                      <Sun size={16} />
                      Light
                    </button>
              ...
              {isUserOpen && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden z-50 backdrop-blur-md">
                  <div className="py-2">
                    <Link
                      to="/profile"
                      className={`flex items-center gap-3 px-4 py-3 text-sm transition-all duration-300 transform hover:scale-105 ${
                        activeLink === "/profile"
                          ? "text-blue-600 bg-blue-50 font-medium border-l-4 border-blue-500"
                          : "text-gray-700 hover:bg-gray-50 hover:text-blue-600"
                      }`}
                      onClick={() => {
                        handleLinkClick("/profile")
                    ...
                    <button
                      onClick={handleLogout}
                      className="flex items-center gap-3 w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 hover:text-red-700 transition-all duration-300 group transform hover:scale-105"
                    >
                      <LogOut size={18} className="text-red-500 group-hover:text-red-600" />
                      <span className="font-medium">Logout</span>
                    </button>
        ...
        <div
          className={`
            flex-1 pt-16 transition-all duration-300 ease-in-out overflow-auto
